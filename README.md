...
- `.htaccess` - настройка поведения веб-сервера
- `config.ini` - настройки конфигурации приложения
- `index.php` - основной файл, точка входа. На него перенаправляются все входящие запросы, которые далее передаются в контроллеры
- `setup.php` - стартовое создание таблиц для моделей БД
- `src/app/` - базовая директория проекта, содержит весь код
